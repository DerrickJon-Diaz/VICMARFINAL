...
export function getPropertyUnitEntries(info = {}) {
  ...
  // For triplex properties, prefer ordering A, C, B instead of A, B, C
  try {
    const normalizedType = String(info?.type ?? info?.property_type ?? info?.title ?? "").toLowerCase();

    if (normalizedType.includes("triplex")) {
      const hasA = entries.some((e) => String(e.unitKey).toUpperCase() === "A");
      const hasB = entries.some((e) => String(e.unitKey).toUpperCase() === "B");
      const hasC = entries.some((e) => String(e.unitKey).toUpperCase() === "C");

      if (hasA && hasB && hasC) {
        const byKey = new Map(entries.map((e) => [String(e.unitKey).toUpperCase(), e]));
        const desiredOrder = ["A", "C", "B"];
        const reordered = [];

        for (const k of desiredOrder) {
          if (byKey.has(k)) {
            reordered.push(byKey.get(k));
            byKey.delete(k);
          }
        }
        ...
        for (const e of entries) {
          const key = String(e.unitKey).toUpperCase();
          if (byKey.has(key)) {
            reordered.push(e);
            byKey.delete(key);
          }
    ...
    // fall back to original entries on error
  }
