...
const SUPPORT_NAV_ITEMS = [
  {
    page: "SupportAdminOverview",
    label: "Overview",
    icon: LayoutDashboard,
  },
  {
    page: "SupportAdminVicinityStatus",
    label: "Vicinity Status",
    icon: MapPinned,
  },
  {
    page: "AdminMessages",
...
export default function SupportAdminLayout({ currentPageName, children }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      const validSupportUser = (isSupportAdminUser(user) || isPrimaryAdminUser(user)) ? user : null;
  ...
  const activePage = useMemo(() => currentPageName ?? "SupportAdminOverview", [currentPageName]);

  const pageContent = useMemo(() => {
    switch (activePage) {
      case "SupportAdminOverview":
        return {
          title: "Support Overview",
          subtitle: "Monitor live chat load and current vicinity slot status at a glance.",
        };
      case "SupportAdminVicinityStatus":
        return {
          title: "Vicinity Status",
          subtitle: "View real-time slot statuses from the vicinity map in read-only mode.",
        };
      case "AdminMessages":
        return {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-[#f0f9ff]/30 to-emerald-50/60 flex relative overflow-hidden">
      ...
      <aside className="hidden lg:flex flex-col fixed top-0 left-0 h-screen z-50 w-[240px] bg-white/80 backdrop-blur-xl border-r border-white/50 shadow-[4px_0_24px_-12px_rgba(0,0,0,0.1)]">
        <div className="flex items-center gap-3 h-20 border-b border-white/40 px-4">
          <Link to={createPageUrl("SupportAdminOverview")} className="flex items-center gap-2.5 group">
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 z-40 h-16 bg-white/80 backdrop-blur-xl border-b border-white/50 shadow-sm flex items-center justify-between px-4">
        <div className="flex items-center gap-3">
          ...
          <Link to={createPageUrl("SupportAdminOverview")} className="flex items-center gap-2">
