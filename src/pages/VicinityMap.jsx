...
import { MapPin, ZoomIn, ZoomOut, Maximize2, Minimize2, X } from "lucide-react";
...
import rowhouseSocializedImage from "@/images/properties/Rowhouse Socialized.png";
...
const bundledFloorPlanImages = {
  ...Object.fromEntries(
    Object.entries(import.meta.glob("../../FloorPlan Image/*", { eager: true, import: "default" }))
      .map(([path, url]) => [path.split("/").pop(), url]),
  ),
  ...Object.fromEntries(
    Object.entries(import.meta.glob("../images/floor_plan/*", { eager: true, import: "default" }))
      .map(([path, url]) => [path.split("/").pop(), url]),
  ),
};
...
function getUnitInfo(property, slotStatuses) {
  return getPropertyUnitEntries(property.info).map((unitEntry) => {
    ...
    // For triplex properties, always use simplified unit type names without letter designations
    const basePropertyType = String(property.info?.type ?? "").trim().toLowerCase();
    let effectiveType;
    
    if (basePropertyType === "triplex") {
      // Explicitly name triplex end units as A and B for accurate labeling
      const unitKey = String(unitEntry.unitKey).toUpperCase();
      if (unitKey === "A") {
        effectiveType = "Triplex (End Unit A)";
      } else if (unitKey === "B") {
        effectiveType = "Triplex (End Unit B)";
      } else if (unitKey === "C") {
        effectiveType = "Triplex (Center Unit)";
      } else {
        effectiveType = "Triplex";
      }
    } else {
      // For non-triplex properties, use override or fallback
      effectiveType = String(slotOverride.type ?? slotOverride.propertyType ?? slotOverride.property_type ?? "").trim()
...
function resolveDetailPropertyIdByUnitType(rawType) {
  ...
  const exactTypeToIdMap = {
    ...
    "triplex (end unit)": "triplex-end-unit-a",
  ...
  if (normalizedType.includes("triplex") && normalizedType.includes("end unit b")) {
    return "triplex-end-unit-b";
  }

  if (normalizedType.includes("triplex") && normalizedType.includes("end unit a")) {
    return "triplex-end-unit-a";
  }

  if (normalizedType.includes("triplex") && normalizedType.includes("end")) {
    return "triplex-end-unit-a";
  }
...
function isCornerSlotType(rawType) {
  return normalizeMatchText(rawType).includes("corner");
}

function isPremiereUnitType(rawType) {
  const normalizedType = normalizeMatchText(rawType);

  return normalizedType.includes("premiere")
    && (normalizedType.includes("duplex") || normalizedType.includes("single attached unit") || normalizedType.includes("single attached duplex"));
}

function isDeluxeUnitType(rawType) {
  const normalizedType = normalizeMatchText(rawType);

  return normalizedType.includes("deluxe")
    && (normalizedType.includes("duplex") || normalizedType.includes("single attached unit") || normalizedType.includes("single attached duplex"));
}

function resolveVicinityPropertyTypeImage(target, slotStatuses) {
  if (!target?.prop) {
    return null;
  }
  ...
  const propertyType = String(target.prop.property_type ?? target.prop.info?.type ?? "").trim().toLowerCase();
  const title = String(target.prop.title ?? target.prop.name ?? "").trim().toLowerCase();
  const propertyId = String(target.prop.id ?? "").trim().toLowerCase();
  const unitKey = String(target.unit?.key ?? "").trim().toUpperCase();

  if (isCornerSlotType(displayType) || isCornerSlotType(propertyType) || isCornerSlotType(title)) {
    return null;
  }

  if (propertyType === "duplex" || normalizedDisplayType.includes("duplex") || title.includes("duplex") || normalizedDisplayType.includes("single attached unit") || title.includes("single attached unit") || normalizedDisplayType.includes("single attached duplex") || title.includes("single attached duplex")) {
    if (isDeluxeUnitType(displayType) || isDeluxeUnitType(propertyType) || isDeluxeUnitType(title)) {
      return duplexDeluxeImage;
    }

    if (isPremiereUnitType(displayType) || isPremiereUnitType(propertyType) || isPremiereUnitType(title)) {
      return duplexPremiereImage;
    }
  }

  if (propertyType === "rowhouse" || normalizedDisplayType.includes("rowhouse") || title.includes("rowhouse")) {
    if (title.includes("compound") || normalizedDisplayType.includes("compound")) {
      return rowhouseCompoundImage;
    }

    if (title.includes("economic") || normalizedDisplayType.includes("economic")) {
      return rowhouseEconomicImage;
    }

    if (title.includes("socialized") || normalizedDisplayType.includes("socialized")) {
      return rowhouseSocializedImage;
    }
  }

  if (propertyType === "triplex" || normalizedDisplayType.includes("triplex") || title.includes("triplex")) {
    // Prioritize unitKey for accurate identification
    if (unitKey === "A") {
    ...
    // Fallback to type-based checks
    if (normalizedDisplayType.includes("center") || propertyId.includes("triplex-center-unit") || title.includes("center unit")) {
      return triplexCImage;
    }
...
function getDisplayType(target, slotStatuses) {
  ...
  // If a specific unit is selected, get its type from getUnitInfo
  if (target.unit?.key) {
    const units = getUnitInfo(target.prop, slotStatuses);
    const selectedUnit = units.find((u) => u.key === target.unit.key);
    if (selectedUnit && selectedUnit.type) {
      return isVacantType(selectedUnit.type) ? (inferTypeFromCategory(target.prop.category) || selectedUnit.type) : selectedUnit.type;
    }
  }
...
function resolveFloorPlanImageUrl(imageName) {
  ...
  if (bundledFloorPlanImages[trimmedName]) {
    return bundledFloorPlanImages[trimmedName];
  }

  const lowerCasedMatch = Object.entries(bundledFloorPlanImages).find(([name]) => name.toLowerCase() === trimmedName.toLowerCase());

  if (lowerCasedMatch?.[1]) {
    return lowerCasedMatch[1];
  }
...
export default function VicinityMap() {
  ...
  const selectedImageUrl = useMemo(() => {
    ...
    if (isCornerSlotType(getDisplayType(selectedTarget, slotStatuses))) {
      return null;
    }

    const imageAsset = resolveVicinityPropertyTypeImage(selectedTarget, slotStatuses);

    if (imageAsset) {
      return imageAsset;
    }

    const imageName = selectedCatalogProperty?.images?.main || null;
  ...
  }, [selectedTarget, slotStatuses, selectedCatalogProperty]);
  ...
  const selectedFloorPlansForModal = useMemo(() => {
    ...
    return [...orderedPlans, ...remainingPlans]
      .map(({ key, plan }) => {
        ...
        const imageUrl = resolveFloorPlanImageUrl(imageName);
  ...
  const handleViewUnit = (selected) => {
    ...
    const targetUnitType = selected?.unit?.type || selectedType;
    const targetUnitId = resolveDetailPropertyIdByUnitType(targetUnitType);
    ...
    if (targetUnitId) {
      params.set("unit", targetUnitId);
    }

    navigate(`${createPageUrl("PropertyTypeUnits")}?${params.toString()}`);
  };
          ...
          {hoveredTarget?.prop && !selectedTarget && (
            ...
            >
              {(() => {
                ...
                const imageUrl = isCornerSlotType(displayType)
                  ? null
                  : (resolveVicinityPropertyTypeImage(hoveredTarget, slotStatuses) || fallbackPropertyImage);
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <h3 className="text-sm font-bold text-slate-900 mb-2">
                          {displayType}
                          {hoveredTarget.unit?.key && !displayType.toLowerCase().includes("triplex") ? ` · Unit ${hoveredTarget.unit.key}` : ""}
                        </h3>
          ...
          >
            {/* Modal Header */}
              ...
              <h3 className="text-xl font-bold text-white">
                ...
                {selectedTarget.unit?.key && !getDisplayType(selectedTarget, slotStatuses).toLowerCase().includes("triplex") ? ` · Unit ${selectedTarget.unit.key}` : ""}
              </h3>
            </div>
