...
export default function AdminSlots({ readOnly = false }) {
      ...
      <div className="space-y-6">
        ...
        <section className="bg-white/60 backdrop-blur-xl rounded-3xl border border-white/60 shadow-[0_4px_24px_rgb(0,0,0,0.03)] overflow-hidden">
          <div className="border-b border-white/40 px-6 py-5 bg-white/40">
            ...
            <h2 className="text-lg font-bold text-slate-800">{readOnly ? "View Slot Availability on Map" : "Select Slot Availability on Map"}</h2>
            <p className="text-sm text-slate-400 mt-0.5">
              {readOnly
                ? "Click any lot area to view the latest slot status details."
              ...
              <div className="rounded-2xl border border-white/60 bg-white/50 overflow-hidden shadow-inner flex flex-col">
                {selectedProperty ? (
                          ...
                          <div className="flex items-center gap-2">
                            {readOnly ? (
                              <div className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700">
                                <span className={`w-2 h-2 rounded-full ${slot.currentMeta.dotClass}`} />
                                {slot.currentMeta.label}
                              </div>
                            ) : (
                              <>
                  ...
                  <div className="flex flex-col items-center justify-center gap-3 p-10 text-center">
                    ...
                    <p className="text-sm text-slate-400">{readOnly ? "Select a slot area on the map to view status." : "Select a slot area on the map to edit availability."}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="bg-[#15803d] text-white">
                  ...
                  {!readOnly ? <th className="text-left font-bold px-5 py-4 text-xs tracking-wider">Edit</th> : null}
                </tr>
              </thead>
